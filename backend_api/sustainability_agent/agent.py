...
from report_processor.firebase_helper import (
    retrieve_data_by_keyword,
)
...
async def describeProducts(file: UploadFile = File(...), userMedicalAilments: str = Form("None")):
    try:
        file_content = await file.read()

        os.makedirs("media/videos", exist_ok=True)
        # Preserve original file extension (image or video)
        ext = "jpg"
        if file.filename and "." in file.filename:
            ext = file.filename.rsplit(".", 1)[-1].lower()
        elif file.content_type:
            if "image/png" in file.content_type:
                ext = "png"
            elif "image/jpeg" in file.content_type or "image/jpg" in file.content_type:
                ext = "jpg"
            elif "video" in file.content_type:
                ext = "mp4"
        temp_path = f"media/videos/uploaded_product.{ext}"
        with open(temp_path, 'wb') as jammer:
            jammer.write(file_content)

        product_details = eval(typeDocInputNOutputFormat(model, product_description_template, EdibleDataExtraction, temp_path))
        ...
        for query in search_queries:
            print(query)
            search_result = tavilySearch(query)
            context.extend(search_result if search_result else [])
        ...
        pros_and_cons_health = eval(getOutPutInFormat(model, enviromental_suggestions.render(
            ...
            user_medical_report_details='\n\n'.join(retrieve_data_by_keyword('Om123') or ['No medical reports on file.'])
        ), [], HealthProsAndCons))
